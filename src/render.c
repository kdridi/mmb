...
static void rotateCamera(float dax, float day) {
	...
	if (render.camera.ax < 0.1)
		render.camera.ax = 0.1;
	if (render.camera.ax > 3.1)
		render.camera.ax = 3.1;
...
void renderDebug() {
	printf("===== RENDER DEBUG =====\n");
	printf("Camera position: %1.1f/%1.1f/%1.1f (int: %i/%i/%i )\n", render.camera.x, render.camera.y, render.camera.z, render.camera.pos.x, render.camera.pos.y, render.camera.pos.z);
	printf("Camera orientation: %1.1f/%1.1f/%1.1f (angle: %1.1f %1.1f )\n", render.camera.dx, render.camera.dy, render.camera.dz, render.camera.ax, render.camera.ay);
...
void renderHookMouse(void (*func)(int button, int state, void *data), void *data) {
	render.onMouse = func;
